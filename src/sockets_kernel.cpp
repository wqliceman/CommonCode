...
// ----------------------------------------------------------------------------------------
    ...
    ) const
    {
        ...
        FD_SET(connection_socket.val, &read_set);
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        ...
        if (timeout > 0)
        {
            ...
            // add the listening socket to read_set
            FD_SET(listening_socket.val, &read_set);
...
// ----------------------------------------------------------------------------------------
		...
		)
	{
		...
		sin >> *this;
		if (!sin || sin.peek() != EOF)
			throw "invalid network address: " + full_address;
	}
		...
		)
	{
		...
		try
		{
			item.port = atoi(temp.substr(pos + 1).c_str());
		}
		catch (std::exception&)
		{
			in.setstate(std::ios::badbit);
		...
		)
	{
		...
		else
		{
			if (hostname_to_ip(host_or_ip, ip))
				throw "unable to resolve '" + host_or_ip + "' in connect()";
		}
